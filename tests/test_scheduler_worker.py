...
from paddock.scheduler_worker import SchedulerWorkerError, SchedulerWorkerManager
...
class Runner:
    ...
    def __call__(self, command, **_kwargs):
        ...
        if command[0] == "journalctl":
            return subprocess.CompletedProcess(command, 0, "tick\ncomplete\n", "")
...
class SchedulerWorkerTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.manager = SchedulerWorkerManager(self.store, self.runner)

    def tearDown(self) -> None:
        self.temporary.cleanup()

    def test_start_projects_selected_php_scheduler_and_logs(self) -> None:
        worker = self.manager.control("start", "demo")
        unit = (self.manager.unit_directory / worker.unit).read_text(encoding="utf-8")
        self.assertEqual("paddock-scheduler-demo.service", worker.unit)
        self.assertIn(str(self.root / "artisan"), unit)
        self.assertIn("schedule:work --no-interaction", unit)
        self.assertNotIn("queue:work", unit)
        self.assertIn(str(self.root.parent / "php-8.4/bin/php"), unit)
        self.assertIn(["systemctl", "--user", "start", worker.unit], self.runner.calls)
        self.assertEqual(("tick", "complete"), self.manager.logs("demo"))

    def test_non_laravel_site_is_refused(self) -> None:
        (self.root / "artisan").unlink()
        with self.assertRaisesRegex(SchedulerWorkerError, "not a detected Laravel"):
            self.manager.configure("demo")

    def test_remove_deletes_only_scheduler_state_and_unit(self) -> None:
        worker = self.manager.configure("demo")
        self.manager.remove("demo")
        self.assertFalse((self.manager.unit_directory / worker.unit).exists())
        self.assertNotIn("scheduler", self.store.read("sites")["sites"]["demo"])
